...
class ScaffCalc {
  ...
  static calcScaffolding({ height, width, stairs, ladders, farfromwall }) {
    ...
    function get3mStandardsForHeight() {
      return parseInt(
        (height + ScaffCalc.guardRailHeight) / ScaffCalc.threeMeterStandards
      );
    }
    ...
    function get1mStandardsForHeight() {
      ...
      return parseInt(height + ScaffCalc.guardRailHeight - accountedHeight);
    }
    ...
    function getStandards2mWithoutGuardrail() {
      const accountedHeight =
        getStandards3mWithoutGuardrail() * ScaffCalc.threeMeterStandards;
      return parseInt((height - accountedHeight) / ScaffCalc.twoMeterStandards);
    }
    ...
    function getTotalShortbeam() {
      return parseInt(getNumberOfStandardPairs() * getShortbeamPerSection());
    }
    ...
    function calcTotalStandards100cm() {
      ...
      const forOuterStandards1m =
        (getNumberOfStandardPairs() + ScaffCalc.innerAndOuterStandards) *
    ...
    function calcTotalStandards200cm() {
      ...
      switch (true) {
        case stairs:
          return (
            forOuterStandards2m + forInnerStandards2m + forStairsStandards2m
          );
        default:
    ...
    function calcTotalStandards300cm() {
      ...
      switch (true) {
        case stairs:
          return (
            forOuterStandards3m + forInnerStandards3m + forStairsStandards3m
          );
        default:
    ...
    function calcTotalStartCollars() {
      ...
      switch (stairs) {
        ...
        default:
          return getNumberOfStandardPairs() * ScaffCalc.innerAndOuterStandards;
      }
    }
    function calcTotalBasePlates() {
      switch (true) {
        ...
        default:
          return getNumberOfStandardPairs() * ScaffCalc.innerAndOuterBeam;
      }
    }
    function calcTotalLedgers73cm() {
      ...
      const shortbeamForPlatformLvls = ScaffCalc.totalLevelsWithPlatforms * 4;
      ...
      switch (true) {
        case ladders && stairs:
          if (getNumberOfSections() > 1) {
            return uBeamForPlatforms + uBeamForStairs + uBeamForLadders;
          } else {
    ...
    function calcTotalSteelDecks() {
      ...
      switch (true) {
        case stairs && ladders:
          return (
            getPlatforms() - replacedPlatformsWithLadder + platformsForStairLvl
          );
        case stairs:
    ...
    function calcScaffoldingPallets() {
      ...
      const totalLedgers = calcTotalLedgers207cm() + calcTotalLedgers307cm();
